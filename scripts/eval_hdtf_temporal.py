"""
eval_hdtf_temporal.py

SyncNetTemporal 평가 스크립트.
체크포인트 내 config에서 모델 구조 자동으로 읽음.
...
def load_model(ckpt_path, device):
    ...
    print(f"  temporal={temporal_cfg.get('type','none')}  pooling={pooling}", flush=True)
    return model
        ...
        for name in tqdm(valid, desc="Eval"):
            ...
            for _ in range(samples_per_utt):
                ...
                lips = torch.tensor(
                    ...
                    dtype=torch.float32,
                ).unsqueeze(0).to(device)  # (1, N, 4, 12, 12)

                mel_pos = get_mel(mel, fidx, t0).unsqueeze(0).to(device)

                v_embed = model.forward_lip(lips)       # (1, emb_dim)
...
def plot_offset_curve(results, out_path, title="SyncNetTemporal — Offset Curve"):
    try:
...
def main():
    ...
    parser.add_argument("--ckpt",            default="logs/syncnet_hdtf_tcn/checkpoints/best.pth")
    parser.add_argument("--latent_root",     default="/media/HDD/jiweon/latents/hdtf_s1")
    parser.add_argument("--data_root",       default="/media/HDD/jiweon/processed/HDTF/HDTF_processed")
    ...
    parser.add_argument("--out",             default="logs/syncnet_hdtf_tcn/eval_offset.png")
    ...
    plot_offset_curve(
        results,
        out_path = args.out,
        title    = f"SyncNetTemporal (TCN) — Offset Curve",
    )
