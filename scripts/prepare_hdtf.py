# /home/jiweon/projects/lip-sync-score/scripts/prepare_hdtf.py
...
def extract_lips(
    ...
    smooth_alpha=0.4,
    min_side_px=20,
    fallback_mode="prev_bbox",   # "prev_bbox" | "black"
    coverage_warn_thr=0.95,      # meta에 기록용
):
    """
    Policy (v2):
    - Always write exactly 1 ROI image per video frame index (00000.png ...).
    - If face/landmark fail:
        - use previous bbox crop if available (fallback_mode="prev_bbox")
        - otherwise write black frame
    - Choose best face when multiple faces: largest face bbox area
    """
    ...
    # last good bbox in original frame coords (x1,y1,x2,y2)
    last_bbox = None

    def write_black(path):
    ...
    def crop_by_bbox(frame, bbox):
        ...
        H, W = frame.shape[:2]
        x1c = max(0, min(W - 1, int(x1c)))
        y1c = max(0, min(H - 1, int(y1c)))
        x2c = max(1, min(W, int(x2c)))
        y2c = max(1, min(H, int(y2c)))
    ...
    while True:
        ...
        n_frames += 1
        out_path = os.path.join(roi_out_dir, f"{idx:05d}.png")

        faces = detector.get(frame)
        if len(faces) == 0:
            n_no_face += 1
            # fallback
            if fallback_mode == "prev_bbox" and last_bbox is not None:
                gray = crop_by_bbox(frame, last_bbox)
                if gray is not None:
        ...
        # pick best face: largest bbox area (more stable in multi-face)
        best = None
        best_area = -1.0
        for f in faces:
            ...
            if bb is None or len(bb) < 4:
                continue
            x1b, y1b, x2b, y2b = [float(v) for v in bb[:4]]
            area = max(0.0, (x2b - x1b)) * max(0.0, (y2b - y1b))
            if area > best_area:
                best_area = area
                best = f

        face = best if best is not None else faces[0]
        lm = getattr(face, "landmark_3d_68", None)  # lm106 = getattr(face, "landmark_2d_106", None)
        ...
        gray = crop_by_bbox(frame, (x1c, y1c, x2c, y2c))
        if gray is None:
            # if bbox invalid, fallback to last_bbox/black
            if fallback_mode == "prev_bbox" and last_bbox is not None:
                gray2 = crop_by_bbox(frame, last_bbox)
                if gray2 is not None:
                    cv2.imwrite(out_path, gray2)
                    n_saved += 1
                    n_fallback_prev += 1
                    if prev_gray is not None:
                        m = float(np.mean(np.abs(gray2.astype(np.float32) - prev_gray.astype(np.float32))))
                        motion_vals.append(m)
                    prev_gray = gray2
                else:
                    write_black(out_path)
            ...
            # update last_bbox only on successful crop
            last_bbox = (x1c, y1c, x2c, y2c)
    ...
    coverage = (n_saved / n_frames) if n_frames > 0 else 0.0
    ...
    stats = {
        ...
        "fallback_prev_bbox": int(n_fallback_prev),
        "fallback_black": int(n_fallback_black),
        ...
        "params": {
            ...
            "coverage_warn_thr": float(coverage_warn_thr),
        },
        "warn": "low_coverage" if coverage < coverage_warn_thr else "",
    }
    return stats
...
def is_success(sample_dir, min_roi_frames=5, min_audio_frames=20):
    ...
    for p in [wav_path, mel_path, mfcc_path]:
...
# ---------------------------------------
# Process one HDTF clip mp4
# ---------------------------------------
def process_one_hdtf_mp4(
    mp4_path,
    detector,
    out_root,
    roi_size=96,
    ...
    sample_id = Path(mp4_path).stem  # file name without extension
    ...
    try:
        ...
        # 2) audio.wav
        extract_audio_wav(mp4_path, wav_path, sr=sr)
...
def main():
    ...
    hdtf_root = prep.get("hdtf_root", None)

    if out_root is None:
        raise ValueError("YAML prepare.out 이 필요합니다.")
    if hdtf_root is None:
        raise ValueError("YAML prepare.hdtf_root 이 필요합니다. (HDTF clips root)")
    ...
    mp4_files = sorted(glob.glob(os.path.join(hdtf_root, "*.mp4")))
    if limit:
        mp4_files = mp4_files[: int(limit)]

    print(f"[HDTF] Found mp4 files: {len(mp4_files)}")
    print(f"[HDTF] clips_root: {hdtf_root}")
    print(f"[HDTF] out_root: {out_root}")
    print(f"[HDTF] roi_size={roi_size}, smooth_alpha={smooth_alpha}, sr={sr}")
    print(f"[HDTF] audio_feat: n_fft={n_fft_audio}, hop={hop_length_audio}, win={win_length_audio} (100Hz)")
    ...
    print(f"Done. SUCCESS={ok}/{len(mp4_files)}")
