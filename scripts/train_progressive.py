...
SyncNetProgressive (Local-to-Global Hierarchical Sync) 학습 스크립트.
기존 train.py와 독립적으로 동작. 기존 코드 건드리지 않음.

실행:
    PYTHONPATH=src python scripts/train_progressive.py --config configs/progressive_last.yaml
    PYTHONPATH=src python scripts/train_progressive.py --config configs/progressive_concat.yaml
"""
...
from lipsyncscore.data.dataset_hdtf_progressive_merged import DatasetHDTFProgressiveMerged
...
def build_model(model_cfg: dict, N: int, device: str) -> SyncNetProgressive:
    temporal_cfg    = model_cfg.get("temporal", {}) or {}
    fusion_mode     = str(model_cfg.get("fusion_mode", "last"))
...
def evaluate(model, criterion, loader, device: str, use_amp: bool) -> dict:
    ...
    for batch in loader:
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16,
            ...
            v     = model.forward_lip(lips, jaw, face)
...
def main():
    ...
    out_dir = Path(train_cfg.get("out_dir", "logs/progressive")).expanduser()
    ...
    # ── Dataset ──────────────────────────────
    print("[INFO] DatasetHDTFProgressiveMerged 사용", flush=True)

    train_ds = DatasetHDTFProgressiveMerged(
    ...
    val_ds = DatasetHDTFProgressiveMerged(
        latent_root = data_cfg["latent_root"],  
        data_root         = data_cfg["data_root"],
        N                 = N,
        min_gap           = int(samp.get("min_gap", 5)),
    ...
    # ── Loss ─────────────────────────────────
    loss_name = loss_cfg.get("name", "infonce")
